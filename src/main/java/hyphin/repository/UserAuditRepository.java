...
public interface UserAuditRepository extends JpaRepository<UserAudit, Integer> {
    ...
    @Query("select moduleName from Modules where moduleId = ?1 and elementId = ?2")
    String findModuleName(String moduleID, String elementId);
    ...
    @Query("select elementType from Element where elementID = ?1")
    String findElementType(String elementID);

    @Query("select mediaLocation from Element where elementID = ?1")
    String findMedialocation(String elementID);

    @Query("select learningJourneyId from LearningJourneys where learningJourneyId = 1 and modulePosition = '1' and classification = 'MVP'")
    String findLearningJourneyId();

    @Query("select learningJourneyName from LearningJourneys where learningJourneyId = 1 and modulePosition = '1' and classification = 'MVP'")
